...
def _main():
    ...
    ax.plot(t, ti.linFit(x, 10), label='linFit')
    ax.plot(t, ti.iir_lowpass(x, 1, 10), label='iir_lowpass')
    ax.plot(t, ti.movingMin(x, 10), label='movingMin')
    ax.plot(t, ti.movingMax(x, 10), label='movingMax')
    ax.plot(t, ti.aema(x, 10), label='aema')
    ax.grid(True)
    ax.legend()

    ax2 = fig.add_subplot(212)
    ax2.plot(t, ti.rateOfChange(x, 20), 'x', label='rateOfChange')
