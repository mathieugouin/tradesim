...
def update_dj():
    ...
    h = pd.read_html('https://en.wikipedia.org/wiki/Dow_Jones_Industrial_Average', keep_default_na=False, storage_options=_headers)
    df = h[2]  # 3rd table
...
def update_tsx():
    """Update TSX list"""
    h = pd.read_html('https://en.wikipedia.org/wiki/S%26P/TSX_Composite_Index', keep_default_na=False, storage_options=_headers)
...
def update_sp500():
    """Update SP-500 list"""
    h = pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies', keep_default_na=False, storage_options=_headers)
    df = h[0]  # first table
